    ...
    for i in range(432, 622):
        for j in range(166, 312):
            ...
            if b<=50 and g<=50 and r>=110:
                correctPixelCount = correctPixelCount +1;
    return correctPixelCount;
...
while(True):
    ...
    # Test Input Random
    #chooseDirection = randomplay()
    #if checkPlayRight:
    #    chooseDirection= "D"
    chooseDirection = ""
    if(checkPlayRight() >= 5000):
        chooseDirection = "D"

    if(checkPlayLeft() >= 5000):
        chooseDirection = "A"

    if(checkPlayUp() >= 5000):
        chooseDirection = "W"

    if(checkPlayDown() >= 5000):
        chooseDirection = "S"
